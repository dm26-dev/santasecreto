...
const getName = JSON.parse(localStorage.getItem('secret')) || { user: '', secret: '' }
...
export const App = () => {
  ...
  useEffect(() => {
    localStorage.setItem('secret', JSON.stringify(userName))
  }, [userName])
  ...
  useEffect(() => {
    ...
    const userLs1 = JSON.parse(localStorage.getItem('secret'))

    if (userLs1.user.length > 0) {
      axios.get(urlApi + '/names')
        .then(res => {          
          const pendientes = res.data.pendingNamesforChoose
          const userLs = JSON.parse(localStorage.getItem('secret'))
          ...
          if (!validateUser) {
            ...
            setStep(0)
            localStorage.setItem('secret', JSON.stringify({ user: userLs.user, secret: '' }))
          }
